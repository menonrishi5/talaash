...
| 25 | `migration-25-dedupe-attendance-sessions.sql` | Consolidates any duplicate-date `attendance_sessions` rows (same merge-the-stronger-row logic as `merge_members()`) and (re)adds the missing `unique (session_date)` guarantee — the likely cause of repeated no-show fines for the same practice. | ⬜ |
