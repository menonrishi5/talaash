...
| 27 | `migration-27-announcement-slack-ts.sql` | Stores the Slack message id (`slack_channel`, `slack_ts`) on `attendance_announcements` so re-announce / room change edits the original post instead of posting a new one, and editors can delete it. Pairs with an `attendance-notify` redeploy. | ✅ *(confirmed run 2026-09, function redeployed)* |
